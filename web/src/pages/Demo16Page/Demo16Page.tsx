...
const DEMO16_MUTATION = gql`
  mutation Demo16($input: Demo16Input!) {
    demo16(input: $input) {
      id
      name
      type
...
const Demo16Page = () => {
  ...
  const [demo16, { error }] = useUploadsMutation(DEMO16_MUTATION, {
    onCompleted: (data) => {
      console.log('Files uploaded:', data.demo16)
  ...
  const { context, progress, setProgress, onAbortHandler } =
    useUploadProgress(DEMO16_MUTATION)
